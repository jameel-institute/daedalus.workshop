...
ggsave(
  ...
  plot_r0, height = 7, width = 10
)
```

```{r plot_eta}
eta_means <- vapply(disease_list, function(x) mean(x$eta), numeric(1))
plot_eta <- ggplot() +
  stat_histinterval(
    aes(
      eta_means, fill = after_stat(level)
    ),
    .width = c(0.5, 1.0)
  ) +
    ...
    theme(
      ...
      labs(
        x = "Hospital admission %", y = "Density", fill = "Confidence interval"
      )

ggsave(
  file.path(figs_out, "plot_dist_eta.png"),
  plot_eta, height = 7, width = 10
)
```
...
```{r costs_summary}
data_cost_output <- get_cost_data(
        output, disease_tags, format = "long"
    ) %>%
...
```{r plot_daily_cases_unmit_explainer}
...
ggsave(
  file.path(figs_out, "DO_NOT_SHOW_unmit_daily_infections.png"),
...
pwalk(
  list(measures, curve_measure_labels, reporting_rates),
  .f = function(m, l, reporting) {
    ...
    filename <- glue::glue(file.path(figs_out, "plot_unmit_{m}.png"))
    ggsave(
      filename,
...
ggsave(
  file.path(figs_out, "plot_unmit_cfr_by_age.png"),
  plot_cfr_by_age,
  height = 7, width = 10
)
```
...
pwalk(
  ...
  .f = function(m, l) {
    ...
    filename <- glue::glue(file.path(figs_out, "plot_compare_{m}.png"))
    ggsave(
      filename,
...
pwalk(
  ...
  .f = function(m, l) {
    ...
    filename <- file.path(figs_out, glue::glue("plot_compare_{m}_square.png"))
    ggsave(
      filename,
      plot = plot_epi_curves_compare,
      width = 14, height = 14
...
filename <- glue::glue(file.path(figs_out, "plot_compare_epi_summary.png"))
ggsave(
  filename,
...
data_epi_curve %>%
  ...
  readr::write_csv(
    file = file.path(tables_out, "table_hcap_breaches.csv")
  )
```

<!-- ## Deaths by age group -->

```{r plot_compare_deaths_by_age}
...
ggsave(
  file.path(figs_out, "plot_compare_deaths_by_age.png"),
  plot_compare_deaths_by_age,
  height = 10, width = 25
)
```

```{r plot_compare_deaths_by_age_square}
plot_compare_deaths_by_age <-
  ggplot(data_deaths) +
  stat_interval(
    aes(
...
data_deaths %>%
  ...
  readr::write_csv(
    file = file.path(tables_out, "table_deaths_by_age.csv")
  )
```

<!-- ## Lost life years, future earnings, and GVA -->

```{r plot_compare_gva_loss}
...
ggsave(
  file.path(figs_out, "plot_compare_gva_loss.png"),
  plot_compare_gva_loss,
  height = 10, width = 21
)
```

```{r plot_compare_gva_loss_square}
plot_compare_gva_loss <-
  ggplot(data_cost_output) +
  stat_interval(
    aes(
...
data_cost_output %>%
  ...
  readr::write_csv(
    file = file.path(tables_out, "table_cost_by_response.csv")
  )
```

<!-- ## Economic losses: Absences -->

```{r plot_compare_econ_cost_absences}
...
ggsave(
  file.path(figs_out, "plot_compare_econ_cost_absences.png"),
  plot_compare_econ_cost_absences,
  height = 7, width = 10
)
```
...
```{r plot_compare_econ_cost_closures_square}
...
ggsave(
  file.path(figs_out, "plot_compare_econ_cost_closures.png"),
  plot_compare_econ_cost_closures,
  height = 7, width = 10
)
```
...
data_econ_costs %>%
  ...
  readr::write_csv(
    file = file.path(tables_out, "table_econ_cost_breakdown.csv")
  )
```
